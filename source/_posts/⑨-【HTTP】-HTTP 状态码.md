...
```BASH
#  401      Unauthorized => 请求要求用户的身份认证 【看下可能是账户，密码错误，或者是网络错误，等等，再或者就是写代码就没有带上要求的 token 等通行证】
...
Unauthorized 请求要求用户的身份认证 **没有登录，或者是登录超时啦，token 错误啦等等错误 - 未授权**
原因：看下可能是账户，密码错误，或者是网络错误，等等，再或者就是写代码 header 就没有带上要求的 token 等通行证
...
- 前端找出错误原因：
    ...
    ③ 外链，跨域是否设置 cookie 携带 为 withCredentials: true
...
- 前端解决 403 错误的方法
    ...
    ③ 更多解决方法查看[88](https://blog.csdn.net/Rnger/article/details/82425961)

## 404 找不到【前端错误/后端错误】

服务器无法找到请求的页面或资源。请求资源无法被找到但是可能以后会再次有效。客户端以后发出的请求也是被允许的。
...
- seo 解决方案：
- 前端找原因：
    ① axios 等等交互 config 配置 method 方法名称写错了？
    ② 方法的参数类型与标准不一致？
    ② 方法异常，返回值类型与标准不一致（一般是前端的问题，可能是 post 要写成  get ？？？看不懂什么鬼错误？？）
...
- 前端找错误原因
    ① 500 报错一般是前端的问题，但也不能排除是前端出错了，例如后台报出**序列化错误**，就可能是前端没有设置`content-Type = application/json`
